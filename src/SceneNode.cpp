...
void SceneNode::Update(glm::mat4 projectionMatrix, Camera* camera){
    if(m_object!=nullptr){
        // TODO: Implement here!
        if(m_parent != nullptr){
            Transform parentTrans = m_parent->GetWorldTransform();
            GetWorldTransform().ApplyTransform(parentTrans * GetLocalTransform());
        }else{
            GetWorldTransform().ApplyTransform(GetLocalTransform());
        }
