#if defined(LINUX) || defined(MINGW) || defined(__APPLE__)
    #include <SDL2/SDL.h>
...
Framebuffer::Framebuffer(){
    ...
    std::string fboVertexShader = m_fboShader->LoadShader("./shaders/blackHoleVert.glsl");
    std::string fboFragmentShader = m_fboShader->LoadShader("./shaders/blackHoleFrag.glsl");
...
void Framebuffer::Create(int width, int height){
    ...
    Unbind();
    //m_fboShader->SetUniform1i("u_DiffuseMap", 1);
    ...
    unsigned char *data = stbi_load("./Textures/Noise.png", &pwidth, &pheight, &nrChannels, 0);
    if(data){
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, pwidth, pheight, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
        glGenerateMipmap(GL_TEXTURE_2D);
    }else{
        printf("Noise image not read.\n");
    }
...
void Framebuffer::DrawFBO(){
    ...
    m_fboShader->SetUniform1i("noiseTexture", 1);
